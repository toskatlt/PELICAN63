...
if (isset($_GET['rsa_date'])) { $id = $_GET['rsa_date']; }
if (isset($_POST['rsa_date'])) { $id = $_POST['rsa_date']; }
...
if (!empty($b['ip'])) {
	...
	if ($output[0] == '1') {
		...
		/*
		////////// КПП ///////////////////////////////////////////////////////////////////////
		preg_match('/\КПП=\b(\d{9})/', $text, $kpp);
		if (isset($kpp[1])) {
			if ($b['kpp'] != $kpp[1]) {
				$update_new = "UPDATE `egais` SET `kpp`='".$kpp[1]."' WHERE `id`='".$b['id']."'";
				mysql_query($update_new, $dbcnx);
				$update_log = "DATE ".$b['kpp']." -> ".$kpp[1]." ";
				echo $update_log." - update_log - КПП<br>";
				mysql_query("INSERT INTO `log` (`id_domain_user`,`date`,`table`,`id_object`,`inquiry`) VALUES ('0', '".$date_time."', 'egais', '".$id."', '".$update_log."')", $dbcnx);
			} else {
				echo $b['kpp']." - КПП не изменилось<br>";
			}
		}
		*/
