...
if (isset($_COOKIE['id']) and isset($_COOKIE['hash'])) {
	...
	if($userdata['id_group'] == "1") {				
			...
			echo "<th style='width: 3%;' class='header'>Кассы</th>";
			echo "</tr>";
			echo "</thead>";
			...
			$allMindal = allObjectMindal ($dbcnx);

			foreach ($allMindal as $aO) {	
				$id_object = $aO['id'];
				$object_name = $aO['name']; 
				...
				preg_match('/(\d+)/usi',$object_name, $number);
				echo "<div hidden='true'>".$number[1]."</div><a href='card?id=".$id_object."' title='".$building_address."'>".$object_name."</a></td>"; 
			...
			////////////////////////  IP адрес  ////////////////////////
				...
				if (isset($selectInternetMaskInObject)) {
					...
					echo "<td align=left><div hidden='true'>".$piecesIPRoute."</div> ".$selectInternetMaskInObject.".1</td> ";
				} else {
